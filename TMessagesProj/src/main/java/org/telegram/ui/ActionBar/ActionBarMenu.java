...
public class ActionBarMenu extends LinearLayout {
    ...
    private View exclusiveItem;
    private View exclusiveItemAnchor;
    ...
    public void clearItems() {
        ...
        exclusiveItem = null;
        exclusiveItemAnchor = null;
        removeAllViews();
    }

    public void onMenuButtonPressed() {
        if (exclusiveItem != null && exclusiveItem.getVisibility() == View.VISIBLE) {
            // The compact chat header intentionally has no overflow menu.
            // Do not let a hardware MENU key open a logically visible but
            // visually suppressed item at a 0x0 anchor.
            return;
        }
        int count = getChildCount();
        for (int a = 0; a < count; a++) {
    ...
    public int getItemsMeasuredWidth(boolean ignoreAlpha) {
        if (exclusiveItem != null && exclusiveItem.getVisibility() == View.VISIBLE) {
            return exclusiveItem.getMeasuredWidth();
        }
        int w = 0;
        int count = getChildCount();
        for (int a = 0; a < count; a++) {
    ...
    public int getVisibleItemsMeasuredWidth() {
        if (exclusiveItem != null && exclusiveItem.getVisibility() != View.GONE) {
            return exclusiveItem.getMeasuredWidth();
        }
        int w = 0;
    ...
    public int getVisibleItemsMeasuredWidthWithAlpha() {
        if (exclusiveItem != null && exclusiveItem.getVisibility() == View.VISIBLE) {
            return (int) (exclusiveItem.getMeasuredWidth() * exclusiveItem.getAlpha());
        }
        float w = 0;
        for (int i = 0, count = getChildCount(); i < count; i++) {
            View view = getChildAt(i);
    ...
    /**
     * Shows a single menu item without changing the logical visibility of the
     * remaining items. This lets a chat temporarily use an iOS-like avatar
     * button and restore Telegram's original actions instantly afterwards.
     */
    public void setExclusiveItem(@Nullable View item) {
        setExclusiveItem(item, item);
    }

    public void setExclusiveItem(@Nullable View item, @Nullable View anchor) {
        if (exclusiveItem != item || exclusiveItemAnchor != anchor) {
            exclusiveItem = item;
            exclusiveItemAnchor = anchor;
            updateExclusiveAccessibility();
            requestLayout();
            if (parentActionBar != null) {
                parentActionBar.invalidate();
            }
        }
    }

    public float getExclusiveItemCenterXInParent() {
        if (exclusiveItem == null || exclusiveItem.getParent() != this || exclusiveItem.getVisibility() != View.VISIBLE) {
            return Float.NaN;
        }
        final View anchor = exclusiveItemAnchor != null && exclusiveItemAnchor.getParent() == exclusiveItem
            ? exclusiveItemAnchor : exclusiveItem;
        return getX() + exclusiveItem.getX()
            + (anchor == exclusiveItem ? 0 : anchor.getX()) + anchor.getWidth() / 2f;
    }

    public float getExclusiveItemCenterYInParent() {
        if (exclusiveItem == null || exclusiveItem.getParent() != this || exclusiveItem.getVisibility() != View.VISIBLE) {
            return Float.NaN;
        }
        final View anchor = exclusiveItemAnchor != null && exclusiveItemAnchor.getParent() == exclusiveItem
            ? exclusiveItemAnchor : exclusiveItem;
        return getY() + exclusiveItem.getY()
            + (anchor == exclusiveItem ? 0 : anchor.getY()) + anchor.getHeight() / 2f;
    }

    private void updateExclusiveAccessibility() {
        for (int i = 0, n = getChildCount(); i < n; i++) {
            final View child = getChildAt(i);
            child.setImportantForAccessibility(exclusiveItem == null || child == exclusiveItem
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
            ...
            for (int a = 0, N = getChildCount(); a < N; a++) {
                ...
                if (view instanceof ActionBarMenuItem) {
                    ...
                    if (lp instanceof MarginLayoutParams) {
                        ...
                        if (view == exclusiveItem) {
                            // Keep the full avatar and its 48dp hit target. The
                            // normal -5dp glass compaction clips this item.
                            mlp.leftMargin = 0;
                            mlp.rightMargin = 0;
                        } else {
                            mlp.leftMargin = -dp(5);
        ...
        if (exclusiveItem != null && exclusiveItem.getParent() == this && exclusiveItem.getVisibility() != GONE) {
            measureChildWithMargins(exclusiveItem, widthMeasureSpec, 0, heightMeasureSpec, 0);
            final MarginLayoutParams lp = (MarginLayoutParams) exclusiveItem.getLayoutParams();
            final int measuredWidth = getPaddingLeft() + getPaddingRight()
                + exclusiveItem.getMeasuredWidth() + lp.leftMargin + lp.rightMargin;
            final int measuredHeight = getPaddingTop() + getPaddingBottom()
                + exclusiveItem.getMeasuredHeight() + lp.topMargin + lp.bottomMargin;
            setMeasuredDimension(
                resolveSize(Math.max(0, measuredWidth), widthMeasureSpec),
                resolveSize(Math.max(0, measuredHeight), heightMeasureSpec));
            return;
        }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        if (exclusiveItem != null && exclusiveItem.getParent() == this && exclusiveItem.getVisibility() != GONE) {
            ...
            exclusiveItem.layout(childLeft, childTop,
                childLeft + exclusiveItem.getMeasuredWidth(),
                childTop + exclusiveItem.getMeasuredHeight());
            for (int i = 0, n = getChildCount(); i < n; i++) {
                final View child = getChildAt(i);
                if (child != exclusiveItem) {
