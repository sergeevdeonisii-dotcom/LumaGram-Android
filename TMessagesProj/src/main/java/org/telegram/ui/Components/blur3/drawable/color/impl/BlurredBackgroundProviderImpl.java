...
public class BlurredBackgroundProviderImpl {
    public static BlurredBackgroundProvider mainTabs(Theme.ResourcesProvider resourcesProvider) {
        return new BlurredBackgroundProviderBuilder(resourcesProvider)
            ...
            .setStrokeColorTop(0x18000000, 0x2EFFFFFF)
            .setStrokeColorBottom(0x26000000, 0x16FFFFFF)
            .setShadowColor(0x30000000, 0x10000000)
            .setShadowLayer(dpf2(3.33f), 0, dpf2(1f))
    ...
    private static float liquidAlpha(boolean isDark) {
        return liquidAlpha(isDark ? 0.68f : 0.62f);
    }

    private static float liquidAlpha(float enabledAlpha) {
        return LiteMode.isEnabled(LiteMode.FLAG_LIQUID_GLASS) ? LiteMode.applyLiquidGlassAlpha(enabledAlpha) : 0.76f;
    }
