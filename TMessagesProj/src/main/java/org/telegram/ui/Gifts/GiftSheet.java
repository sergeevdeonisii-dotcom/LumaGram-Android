        ...
        recyclerListView.setOnItemClickListener((view, position) -> {
            ...
            if (item.instanceOf(GiftCell.Factory.class)) {
                if (item.object instanceof GiftPremiumBottomSheet.GiftTier) {
                    ...
                    final boolean archivedGift = StarsController.getInstance(currentAccount).isArchivedGift(gift.id);
                    if (archivedGift || gift.locked_until_date > ConnectionsManager.getInstance(currentAccount).getCurrentTime()) {
                        ...
                        ConnectionsManager.getInstance(currentAccount).sendRequest(req, (res, err) -> AndroidUtilities.runOnUIThread(() -> {
                            ...
                            if (res instanceof TL_stars.checkCanSendGiftResultOk) {
                                ...
                                final AlertDialog dialog = new AlertDialog.Builder(getContext(), resourcesProvider)
                                    .setTitle(getString(archivedGift ? R.string.Gift2SoldOutSheetTitle : R.string.GiftLocked))
